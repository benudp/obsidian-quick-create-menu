...
export class TargetEditModal extends Modal {
  ...
  constructor(
    ...
    this.setTitle(this.isNew ? "Add New Target" : "Edit Target");
  ...
  onOpen() {
    ...
    const conditionalContainer = contentEl.createDiv({
      cls: "quick-note-conditional-settings",
    });
    ...
    new Setting(contentEl).setName("Enabled").addToggle((toggle) =>
      toggle.setValue(this.target.enabled).onChange((value) => {
  ...
  refreshConditionalSettings() {
    ...
    if (this.target.type !== "daily-note") {
      ...
      // Preview
      this.previewDiv = this.conditionalContainer.createDiv({
        cls: "quick-note-preview",
      });
      this.updatePreview();
    }
  }
  ...
  updatePreview() {
    ...
    try {
      ...
      this.previewDiv.innerHTML =
        '<strong>Preview:</strong> <span style="color: var(--text-error);">Invalid date format</span>';
    }
  }
