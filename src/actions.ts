...
export class ActionHandler {
  ...
  showCustomPopup(
    ...
    targets.forEach((target) => {
      ...
      iconSpan.style.color = target.color;
  ...
  async executeCreate(target: NoteTarget) {
    ...
    // Logic: Use new pattern if exists, otherwise fallback to old prefix + date
    let pattern = target.filenamePattern;

    // If pattern is missing (legacy data), construct it from the old prefix
    if (!pattern) {
      // We cast to 'any' because 'prefix' no longer exists on the NoteTarget interface
